...
def plot_experiment(ax, experiment_name, zoom_worker_minimum=None, zoom_worker_maximum=None):
  ...
  for log_file in glob.glob("data/%s/*-0.out" % experiment_name):
    ...
    mean = np.mean(step_times) if local_batch_size == 64 else mean_64
    stddev = np.std(step_times) if local_batch_size == 64 else stddev_64
    mean_64 = mean
    stddev_64 = stddev
    if stddev == 0.0:
  ...
  print(x)
  print(y)
  perfect_scaling = [data[128]] * len(x)
  # Maybe zoom
